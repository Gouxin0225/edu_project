...
public class AiChatPromptBuilder {
    ...
    public String buildUserPrompt(
        ...
        return String.format("""
                ...
                来源说明：%s
                回答执行规则：
                1. 先判断问题是否完整；如果缺少必要条件，先提出1到3个澄清问题，并给出可基于现有信息确认的部分。
                2. 回答平台数据、知识库文档、学生成绩、作业、题库等事实时，只能使用可用参考上下文。
                3. 如果参考上下文没有支持某个事实，要明确说“当前平台数据无法确认”，不要猜测。
                4. 如果问题同时包含平台查询和学习答疑，先回答可确认的平台部分，再补充通用学习讲解。
                5. 涉及步骤、方案或教学建议时，给出可执行的下一步，不只给抽象结论。
    ...
    private String buildStudentSystemPrompt(String lengthRule, String intentRule) {
        ...
        return """
                ...
                7. 对含糊问题要主动追问，例如缺少课程、知识点、班级、任务名称或时间范围时，不要直接假设
                排版规则：
                - 可以使用列表、加粗和分段进行结构化排版。
                - 不要使用 Markdown 标题符号（#、##、### 等），小标题请用加粗短句或编号表达。
                - 不要使用代码块围栏（``` 或 ~~~），也不要把整段回答放进代码块。
                - 如果必须展示命令、配置或短代码片段，请直接用普通文本或行内代码表达，保持可读即可。
                输出建议：
                - 使用清晰的加粗短句或序号
    ...
    private String buildTeacherSystemPrompt(String lengthRule, String intentRule) {
        ...
        return """
                ...
                7. 对含糊问题要主动追问，例如缺少课程、班级、任务名称、学生范围或时间范围时，不要直接假设
                排版规则：
                - 可以使用列表、加粗和分段进行结构化排版。
                - 不要使用 Markdown 标题符号（#、##、### 等），小标题请用加粗短句或编号表达。
                - 不要使用代码块围栏（``` 或 ~~~），也不要把整段回答放进代码块。
